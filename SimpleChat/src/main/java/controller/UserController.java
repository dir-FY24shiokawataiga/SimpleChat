...
import beans.SessionControl;
import dtos.LoginForm;
import dtos.SignupForm;
import entity.User;
...
public class UserController 
{
    ...
    @GetMapping("/signup")
    public ModelAndView showsSignUpPage(@ModelAttribute("signupForm")SignupForm signupForm, ModelAndView mav)
    {
    ...
    {
        User exsitingUser = this.userRepository.findByEmail(signupForm.getMailAddress());
    ...
    public ModelAndView login(@ModelAttribute("loginForm") LoginForm loginForm, ModelAndView mav)
    {
        ...
        {
            mav.setViewName("refirect:/users");
        }
        else 
        {
            mav.setViewName("/login");
